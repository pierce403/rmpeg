...
- Symphonia-backed FLAC decode matches the mirrored FFmpeg framemd5 fixture exactly, but MP3, Vorbis, and AAC-in-MP4 produce deterministic PCM that is not byte-identical to FFmpeg's decoder output. The generated MP3 fixture is closer through optional runtime `libmpg123.so.0` (66 one-LSB sample differences from FFmpeg versus 82 through Symphonia on this machine), but it still is not bitexact. The CLI now uses an exact FFmpeg PCM compatibility backend for many non-FLAC audio rows while the independent decoder math and codec coverage remain incomplete.
